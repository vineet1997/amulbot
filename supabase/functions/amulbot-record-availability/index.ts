...
Deno.serve(async (request) => {
  ...
  try {
    ...
    for (const check of checks) {
      ...
      await fetch(`${PROJECT_URL}/rest/v1/availability_checks?on_conflict=product_sku,pincode`, { method: "POST", headers: { ...dbHeaders(true), Prefer: "resolution=merge-duplicates" }, body: JSON.stringify({ product_sku: check.sku, pincode: check.pincode, is_available: check.available === true, checked_at: checkedAt, detail: check.detail ?? null }) });
      ...
      for (const alert of await activeResponse.json()) {
        ...
        await fetch(`${PROJECT_URL}/rest/v1/notifications`, { method: "POST", headers: dbHeaders(true), body: JSON.stringify({ alert_id: alert.id, availability_check_at: checkedAt, status: result ? "sent" : "failed", telegram_message_id: result?.result?.message_id ?? null }) });
        if (result) { notifications++; await fetch(`${PROJECT_URL}/rest/v1/alerts?id=eq.${alert.id}`, { method: "PATCH", headers: dbHeaders(true), body: JSON.stringify({ last_notified_at: new Date().toISOString() }) }); }
      }
    }
    return Response.json({ ok: true, notifications });
  } catch (error) { console.error("amulbot-record-availability", error); return Response.json({ error: "Unable to record checks" }, { status: 500 }); }
});
